...
본론에 들어가기에 앞서, V4L2 (Video for Linux) 와 Video Buffer 에 대해 간단하게
...
vmalloc 의 경우 커널 가상 메모리 상에서는 연속적이지만 실제 물리적으로는
...
있었다. DMABUF 방식의 사용 목적을 고려하지 않았다는 점이다.

할당자가 변경되어야 하는 배경은 이해했지만, DMABUF 방식은 이해할 수 없었다. '왜
이해가 안되는 것인지'가 머리로 이해가 안되는 답답함에 코드를 보고 있었는데,
...
OVERLAY는 생략하고 나머지를 보면, 먼저 MMAP은 드라이버가 버퍼를 할당하여
...
했을 때 관련된 정보들만 설정할 뿐이지 버퍼 메모리를 할당하지 않는다.

# 끝맺음

다음 주에는 V4L2_MEMORY_MMAP으로 할당한 뒤, 이를 expbuf 를 이용하여 DMABUF
